...
// SD card stuff
//Declaring the instances needed for FAT File System
...
int main(void)
{
	...
	for (int i = 0; i < 16; i++)
	{
		loadFromFileInSDCard(i, (u32)&recordings[i]);
	}
    ...
    while(1) {
		...
		if (Demo.fDmaS2MMEvent)
		{
			...
			Xil_DCacheInvalidateRange(addr, 15*NR_AUDIO_SAMPLES);
			//microblaze_invalidate_dcache();
			// Reset S2MM event and record flag
			Demo.fDmaS2MMEvent = 0;
			Demo.fAudioRecord = 0;

			writeToFileInSDCard(dip_check, addr);
		}
...
//					//microblaze_flush_dcache();
			Xil_DCacheFlushRange(addr, 15*NR_AUDIO_SAMPLES);
			...
			switch(Demo.chBtn) {
				...
				case 'd':
					if (!Demo.fAudioRecord && !Demo.fAudioPlayback)
					{
						...
						fnAudioPlay(sAxiDma,15*NR_AUDIO_SAMPLES, addr);
...
void writeToFileInSDCard(int recordingIndex, u32 addr)
{
	FIL file1;
	char FileName[32];
	char* FileNamePtr;
	...
	uint recordingSize = 5760000;//2097152;
...
void loadFromFileInSDCard(int recordingIndex, u32 addr)
{
	unsigned int bytesR;
	uint recordingSize = 5760000;
